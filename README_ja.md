...
ツール呼び出しをアプリ層で遮断し、不正なアクションをブロックするポリシー適用ツールは、必要な第一防衛線だ。[Microsoft の Agent Governance Toolkit](https://github.com/microsoft/agent-governance-toolkit) などのフレームワークはその役割を十分に果たしている。

このプロジェクトが対象とするのは、**アクションが許可された後**に何が起きるかだ。

インシデントが発生したとき、必要なのは「アクションが許可されたという記録」ではない。その記録自体が改ざんされていないという証明だ——敵対的な監査、プラットフォーム移行、量子コンピューターを前にしても有効な証明が。

耐量子署名（ML-DSA-65）、RFC 3161 信頼タイムスタンプ、7年間の WORM ストレージはそのために存在する。「ガバナンスの主張」と「ガバナンスの証明」の違いがここにある。

ポリシー適用ツールとこのプロジェクトは競合ではなく、補完関係にある。
...
| 指標 | 導入前 | 導入後 | 変化 |
|---|---|---|---|
| AI呼び出しコスト — L5 subprocessフラグ | $0.21/回 | $0.001/回 | −99.3% 実測 |
| SessionStartコンテキスト — L2 | 19 MB | 36 KB | −99.8% 実測 |
| RTK出力圧縮 — L6 | コンテンツトークン | 日次平均−70% | 37日間実測 |
| L5+L6+L7 複合（1コール） | $0.21/回 | ~$0.00009/回 | −99.96% |

> 技術別詳細: [`docs/achievements.md`](docs/achievements.md) · 8層リファレンス: [`docs/token-optimization-layers.md`](docs/token-optimization-layers.md)
...
| 自動化パイプラインのセキュリティ設計 | hookに依存しないゼロトラスト設計（コンテンツ層でのスキャン）が正しいアーキテクチャ。P-004としてロードマップに登録済み。詳細: [`docs/architecture.md`](docs/architecture.md) Diagram 4 |
